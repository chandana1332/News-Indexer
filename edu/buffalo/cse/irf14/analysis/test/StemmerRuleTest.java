...
public class StemmerRuleTest extends TFRuleBaseTest {
	...
	public final void testRule() {
		try {
			...
			assertArrayEquals(
					new String[] { "inflat", "fail", "troubl", "fill" },
					runTest(TokenFilterType.STEMMER,
							"inflated failing troubled filling"));
			...
			assertArrayEquals(
					new String[] { "microscop", "replac", "of", "inher", "good" },
					runTest(TokenFilterType.STEMMER,
							"microscopic replacement of inherent goodness"));
			assertArrayEquals(
					new String[] { "@goodness", "#gracious", "2getherness" },
					runTest(TokenFilterType.STEMMER,
							"@goodness #gracious 2getherness"));
