...
public class SpecialCharRuleTest extends TFRuleBaseTest {
	...
	public void testRule() {
		try {
			...
			assertArrayEquals(
					new String[] { "destructor", "is", "method" },
					runTest(TokenFilterType.SPECIALCHARS,
							"destructor is ~method()")); // tilda, brackets
			assertArrayEquals(
					new String[] { "email", "is", "testbuffalo.edu" },
					runTest(TokenFilterType.SPECIALCHARS,
							"email is test@buffalo.edu")); // @
			...
			assertArrayEquals(new String[] { "x2", "xx" },
					runTest(TokenFilterType.SPECIALCHARS, "x^2 = x*x")); // crows
																			// feet,
																			// asterisk
																			// and
																			// equal
			...
			assertArrayEquals(new String[] { "case", "x", "continue" },
					runTest(TokenFilterType.SPECIALCHARS, "case x: continue;")); // :
																					// ;
			// assertArrayEquals(new String[]{"stdin", "cut", "-f1", "sort",
			// "myfile"},
			// runTest(TokenFilterType.SPECIALCHARS,
			// "stdin < cut -f1 | sort > myfile")); //< > |
